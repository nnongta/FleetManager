...
const StatCards = ({ bookings }: { bookings: Booking[] }) => {
    const stats = [
        { label: "Pending Approval", count: bookings.filter(b => b.status === "Pending").length, color: "text-amber-600" },
        { label: "Active Trips", count: bookings.filter(b => b.status === "Approved").length, color: "text-blue-600" },
        { label: "Completed", count: bookings.filter(b => b.status === "Completed").length, color: "text-green-600" },
        { label: "Total Canceled", count: bookings.filter(b => b.status === "Rejected").length, color: "text-red-600" },
    ];

    return (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            {stats.map((stat, index) => (
                <div key={index} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm transition-hover hover:shadow-md">
    ...
    return (
        <div className="flex flex-col gap-6 p-4 animate-in fade-in duration-500">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 className="text-3xl font-black tracking-tight text-slate-900">Booking Management</h1>
                    <p className="mt-1 text-sm text-slate-500">จัดการและติดตามสถานะการจองรถขององค์กร</p>
                </div>
